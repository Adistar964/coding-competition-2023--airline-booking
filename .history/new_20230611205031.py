...
def main(user):
    ...
    def cancel():
        ...
        number11 = Entry(top, font=font.Font(size=13))
        number11.pack(pady=5)
        def can():
            msg.showinfo("cancelled!", f"Your flight with ticket number: '{number11.get()}' is now cancelled")
            top.destroy()
            for idx,row in df.iterrows():
                if number11.get() == row["ticket_number"]:
