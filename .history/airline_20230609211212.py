...
def main(user,loc):
    ...
    f = Frame(text,padx=160,bg="white")
    ...
    def confirm_book():
        ...
        def book():
            ...
            if trip_type == 1:
                ...
                tim = f"{time} at {aday.get()}/{amonth.get()}"
            df.loc[len(df)] = [user,number,f"{time} at {day.get()}/{month.get()}",tim,fromm.get(),to.get()]
    ...
    def show_book():
        ...
        print(check)
        if check:
            l = Listbox(t,pady=5,bd=3)
            for index,row in df["names"].iterrows():
                if row["name"] == user:
                    l.insert(index,f"{row["departure_time"].replace("00",":").replace("_"," ")}")
        else:
