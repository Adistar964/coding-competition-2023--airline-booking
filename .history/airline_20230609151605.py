...
def main(user,loc):
    ...
    f = Canvas(bg="white",highlightbackground="lightblue",highlightthickness=3)
    scroll = Scrollbar(f,orient="vertical",command=f.yview)
    scroll.pack(fill="y",side="right")
    f.configure(yscrollcommand=scroll.set)
    f.bind("<Configure>",command=lambda e: f.configure(scrollregion=f.bbox("all")))
